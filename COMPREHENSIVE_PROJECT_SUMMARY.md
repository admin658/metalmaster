...
- Guitar grading engine added: `packages/web/src/audio/grading.ts` aligns expected vs played notes with timing/pitch tolerances, coverage, jitter, and overall grade; `GuitarFeedbackEngine` now captures played notes and returns a `GradeSummary`, and `RiffEvaluationResult` surfaces grade/pitch/timing metrics, bias, misses, and extras.
