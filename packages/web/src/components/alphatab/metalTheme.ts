...
export function attachMetalTheme(api: any, alphaTab: any) {
...
export function applyMetalTheme(score: any, alphaTab: any) {
  ...
  for (const track of score.tracks) {
    ...
    for (const staff of track.staves) {
      for (const bar of staff.bars) {
        ...
        barColors.set(
          m.BarSubElement.StandardNotationStaffLine,
          Color.fromJson(METAL_PALETTE.staffLine)
        );
        ...
        barColors.set(
          m.BarSubElement.StandardNotationBarLines,
          Color.fromJson(METAL_PALETTE.muted)
        );
        barColors.set(m.BarSubElement.GuitarTabsBarNumber, Color.fromJson(METAL_PALETTE.accent));
        barColors.set(
          m.BarSubElement.StandardNotationBarNumber,
          Color.fromJson(METAL_PALETTE.accent)
        );
        ...
        barColors.set(
          m.BarSubElement.StandardNotationTimeSignature,
          Color.fromJson(METAL_PALETTE.muted)
        );
        barColors.set(m.BarSubElement.GuitarTabsTimeSignature, Color.fromJson(METAL_PALETTE.muted));
        barColors.set(
          m.BarSubElement.StandardNotationKeySignature,
          Color.fromJson(METAL_PALETTE.muted)
        );
        ...
        for (const voice of bar.voices) {
          ...
          for (const beat of voice.beats) {
            ...
            beatColors.set(
              ...
              Color.fromJson(METAL_PALETTE.secondary)
            );
            beatColors.set(
            ...
            beatColors.set(
              m.BeatSubElement.StandardNotationTuplet,
              Color.fromJson(METAL_PALETTE.accent)
            );
            ...
            for (const note of beat.notes) {
              ...
              noteColors.set(m.NoteSubElement.StandardNotationNoteHead, Color.fromJson(fretColor));
              noteColors.set(
              ...
              // Tab fret number
              noteColors.set(m.NoteSubElement.GuitarTabFretNumber, Color.fromJson(fretColor));

              // Optional: slash / numbered notations
              noteColors.set(m.NoteSubElement.SlashNoteHead, Color.fromJson(fretColor));
              noteColors.set(m.NoteSubElement.NumberedNumber, Color.fromJson(fretColor));
            }
          }
        }
      }
