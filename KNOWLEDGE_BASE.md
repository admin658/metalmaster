...
- Styling: monochrome AlphaTab skin in `src/app/alphaTab.css`; global metal theme via `globals.css` + Geist fonts; Tab Lab forces a white/ink AlphaTab surface with scoped cursor/selection/highlight CSS in `AlphaTabCanvas`.
